...
export interface EventPayloads {
  ...
  // a lost card replaced: the tag it used to carry must stop opening the gate, and the family pays for the new one
  'id_card.reissued': { cardId: string; newCardId: string; personType: 'student' | 'staff'; personId: string; cardNo: string; oldCardNo: string; revokedTag: string | null; reason: string };
  ...
  // money that nobody clicked: what the watchdog jobs in fees, accounting, commerce and giving found
  'discount.expired': { discountId: string; studentId: string; schemeId: string; validTo: string };
  'cheque.overdue': { paymentId: string; studentId: string; amount: number; reference: string; days: number };
  'campaign.closed': { campaignId: string; title: string; raised: number; goal: number };
  // the watchdog over the machinery itself: something that should have run has not, or something
  // that should have been delivered was not. Never carries what was in the message, only that it stalled.
  'automation.stalled': { kind: 'scheduled_job' | 'outbox' | 'backup' | 'storage' | 'notifications'; detail: string; count: number };
  // a plugin's webhook failed often enough that we stopped calling it; the school is told, not retried forever
  'plugin.disabled': { installId: string; slug: string; failures: number; reason: string };
  // money owed to a reseller, worked out and waiting for a person to actually send it
  'payout.due': { payoutId: string; partnerId: string; partner: string; period: string; amount: number };
}
...
export const EVENT_TYPES = Object.freeze([
  ...
  'enquiry.created', 'application.submitted', 'applicant.enrolled', 'campaign.opened', 'test.results_entered', 'merit_list.generated', 'offer.made', 'document.requested', 'document.issued', 'id_card.reissued', 'book.issued', 'book.returned', 'transport.assigned', 'hostel.allocated', 'outpass.applied', 'po.received', 'complaint.created', 'incident.reported', 'course.published', 'assignment.published', 'event.created', 'academic_year.created', 'calendar.holiday_added', 'leave.approved', 'leave.rejected', 'attendance.marked', 'attendance.absent', 'leave.applied', 'punches.ingested', 'ptm.booked', 'diary.published', 'substitution.approved', 'marks.locked', 'result.published', 'exam.scheduled', 'promotion.applied', 'invoice.created', 'invoice.batch_finished', 'journal.posted', 'expense.created', 'discount.proposed', 'payment.received', 'cheque.received', 'cheque.cleared', 'cheque.bounced', 'instalment_plan.created', 'wallet.topped_up', 'pos.sold', 'shop.ordered', 'scholarship.awarded', 'donation.received', 'alumni.graduated', 'competition.results_recorded', 'work_order.raised', 'work_order.done', 'meeting.minuted', 'policy.published', 'election.closed', 'govt_report.generated', 'data_request.made', 'anomaly.detected', 'risk.flagged', 'broadcast.sent', 'subscription.changed', 'saas_invoice.raised', 'plugin.installed', 'ai.generated', 'group.created', 'student.transferred', 'payroll.calculated', 'payroll.approved', 'payroll.paid', 'staff.joined', 'staff.left', 'application.received', 'notice.published', 'message.sent', 'test.ping', 'forecast.cash_projected', 'forecast.staffing_gap', 'ivr.call_received', 'ivr.callback_requested', 'course.registered', 'course.sold', 'program.completed', 'lesson.completed', 'discussion.replied', 'assignment.similarity_flagged', 'revision_plan.built', 'payroll.approval_due', 'staff.settlement_due', 'meeting.minutes_overdue', 'policy.unacknowledged', 'consent.expired', 'safeguarding.review_due', 'vehicle.unfit', 'hostel.rollcall_missing', 'stock.below_reorder', 'gate_pass.outstanding', 'discount.expired', 'cheque.overdue', 'campaign.closed', 'automation.stalled', 'plugin.disabled', 'payout.due',
