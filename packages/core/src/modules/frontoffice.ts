    ...
    return {
      ...
      'frontoffice.gate_watch': async ({ schoolId, payload }) => {
        ...
        for (const p of passes) {
          ...
          await this.notifications.notifyRoleOnce(schoolId, 'admin', 12, { channels: ['push', 'in_app'], eventKey: 'frontoffice.pass_outstanding', title: 'A gate pass has not been closed', body: `${who} was due back at ${String(p.expected_in).slice(0, 16)} and the pass is still open. Find out where they are before anybody is told anything.`, entityType: 'frontoffice.gate_pass', entityId: String(p.id) });
        ...
        if (visitors.length) {
          ...
          await this.notifications.notifyRoleOnce(schoolId, 'admin', 20, { channels: ['in_app', 'push'], eventKey: 'frontoffice.visitors_open', title: `${visitors.length} visitor badge(s) are still open`, body: `${list.slice(0, 500)}. Nothing has been signed out for them: check the badges are back and close the book by hand.`, entityType: 'frontoffice.visitor_book', entityId: onDate });
