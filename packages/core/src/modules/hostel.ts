    ...
    return {
      ...
      'hostel.night_watch': async ({ schoolId, payload }) => {
        ...
        for (const h of hostels) {
          ...
          if (warden?.user_id) await this.notifications.notifyOnce(20, { schoolId, userId: String(warden.user_id), channels: ['push', 'in_app', 'sms'], eventKey: 'hostel.rollcall_missing', title: 'The night roll call has not been taken', body, entityType: 'hostel.rollcall', entityId: String(h.id) });
          await this.notifications.notifyRoleOnce(schoolId, 'admin', 20, { channels: ['push', 'in_app'], eventKey: 'hostel.rollcall_missing', title: 'The night roll call has not been taken', body, entityType: 'hostel.rollcall', entityId: String(h.id) });
          const wardenUser = (warden?.user_id as string) ?? null;   // a task is held by a user account, not a staff row
          await this.tasks.ensure({ schoolId, title: `Take the night roll call in ${h.name} (${onDate})`, taskType: 'hostel.rollcall', assignedTo: wardenUser, assignedRole: wardenUser ? null : 'admin', entityType: 'hostel.rollcall', entityId: String(h.id), priority: 'urgent' });
