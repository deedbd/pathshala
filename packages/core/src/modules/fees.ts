...
export class FeesService {
  ...
  /**
   * This month's money against what was invoiced, in three queries whatever the size of the school.
   *
   * `invoiced` is what the month's own bills add up to (`billing_period`, which is what a fee month
   * means here), `collected` is the cash that arrived inside the month's own window, and the two are
   * deliberately not the same question — a payment in September against an August bill belongs to
   * September's takings and August's billing, and pretending otherwise is how a collection rate ends
   * up over a hundred per cent. `outstanding` and `overdue` come off the invoice balances, so they
   * agree with the ledger the accountant reads.
   *
   * `windows` are the months to chart, oldest first, each with the UTC window its local month
   * occupies; the last of them is the month the page is about. The month buckets are `CASE` arms in
   * the one payments query rather than a query per month, because a dashboard on shared hosting
   * cannot afford nine round trips for a nine-column chart.
   */
  async overview(schoolId: string, opts: { windows: { month: string; from: string; to: string }[]; today: string; dayRange: { from: string; to: string } }) {
    const windows = opts.windows;
    if (!windows.length) throw badRequest('at least one month is needed');
    const current = windows[windows.length - 1]!;
    const arms = windows.map(() => `COALESCE(SUM(CASE WHEN paid_at BETWEEN ? AND ? THEN amount ELSE 0 END), 0)`);
    const bounds = [windows[0]!.from, opts.dayRange.from].sort()[0]!;
    const [pay] = await this.db.query<Row>(
      `SELECT COALESCE(SUM(CASE WHEN paid_at BETWEEN ? AND ? THEN amount ELSE 0 END), 0) AS today_total, ${arms.map((a, i) => `${a} AS m${i}`).join(', ')}
       FROM payments WHERE school_id = ? AND status = 'success' AND paid_at BETWEEN ? AND ?`,
      [opts.dayRange.from, opts.dayRange.to, ...windows.flatMap(w => [w.from, w.to]), schoolId, bounds, [current.to, opts.dayRange.to].sort().slice(-1)[0]!]);
    // one grouped pass over the invoices gives both the per-month billing and the totals the office
    // chases: a school's whole billing history is a few hundred rows, one per month it has ever run
    const billed = await this.db.query<Row>(`SELECT billing_period,
        COALESCE(SUM(total), 0) AS invoiced,
        COALESCE(SUM(CASE WHEN balance > 0 THEN balance ELSE 0 END), 0) AS outstanding,
        COALESCE(SUM(CASE WHEN balance > 0 AND due_date < ? THEN balance ELSE 0 END), 0) AS overdue,
        COALESCE(SUM(CASE WHEN balance > 0 AND due_date < ? THEN 1 ELSE 0 END), 0) AS overdue_invoices
      FROM invoices WHERE school_id = ? AND status <> 'cancelled' GROUP BY billing_period`, [opts.today, opts.today, schoolId]);
    const invoicedBy = new Map<string, number>();
    let outstanding = 0, overdue = 0, overdueInvoices = 0;
    for (const b of billed) {
      const month = b.billing_period == null ? null : String(b.billing_period).slice(0, 7);
      if (month) invoicedBy.set(month, round((invoicedBy.get(month) ?? 0) + Number(b.invoiced)));
      outstanding = round(outstanding + Number(b.outstanding));
      overdue = round(overdue + Number(b.overdue));
      overdueInvoices += Number(b.overdue_invoices);
    }
    const [batch] = await this.db.query<Row>(`SELECT id, invoice_count, generated_by, status, started_at, finished_at, created_at
      FROM invoice_batches WHERE school_id = ? ORDER BY created_at DESC, id DESC LIMIT 1`, [schoolId]);
    return {
      month: current.month,
      invoiced: invoicedBy.get(current.month) ?? 0,
      collected: round(Number(pay?.[`m${windows.length - 1}`] ?? 0)),
      collectedToday: round(Number(pay?.today_total ?? 0)),
      outstanding, overdue, overdueInvoices,
      byMonth: windows.map((w, i) => ({ month: w.month, invoiced: invoicedBy.get(w.month) ?? 0, collected: round(Number(pay?.[`m${i}`] ?? 0)) })),
      // a batch nobody generated by hand was generated by the scheduler: that is what "automatic" means
      lastBatch: batch ? { at: String(batch.finished_at ?? batch.started_at ?? batch.created_at), invoices: Number(batch.invoice_count ?? 0), automatic: batch.generated_by == null } : null,
    };
  }
