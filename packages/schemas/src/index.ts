...
export const pageBlockSchema = z.object({ type: z.enum(['hero', 'text', 'notices', 'admission_cta', 'gallery', 'contact', 'stats', 'results_lookup']), title: z.string().max(200).optional(), titleBn: z.string().max(200).optional(), body: z.string().max(20000).optional(), bodyBn: z.string().max(20000).optional(), image: z.string().max(500).optional().nullable(), cta: z.object({ label: z.string().max(80), href: z.string().max(300) }).optional().nullable(), limit: z.number().int().min(1).max(50).optional() });
