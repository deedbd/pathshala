...
export function chipClass(status: string | null | undefined): string {
  switch ((status ?? '').toLowerCase()) {
    case 'success': case 'done': case 'sent': case 'delivered': case 'active': case 'approved': case 'paid': case 'present': return 'chip chip-ok';
    case 'pending': case 'queued': case 'running': case 'scheduled': case 'partial': case 'late': case 'preview': return 'chip chip-warn';
    case 'failed': case 'rejected': case 'overdue': case 'absent': case 'cancelled': case 'locked': return 'chip chip-bad';
    case 'skipped': case 'inactive': case 'draft': return 'chip';
