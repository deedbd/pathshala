...
try {
  ...
  // The school's own address, read through its own settings API rather than the vendor console: the
  // account the installer just made administers this school, and the owner console belongs to
  // whoever comes through the owner door — asking it here would 404, exactly as it should.
  const web = await api('/api/school/web', undefined, 'GET', cookie);
  ...
  // Only now are the pages worth looking at: until the installer is finished every address that is
  // not /install or /api redirects to the wizard, so a page checked before this point answers 200
  // with the installer's own HTML and proves nothing. This check used to run above it and did.
  for (const p of ['/site', '/portal/login', doorPath]) {
    ...
    if (r.redirected || new URL(r.url).pathname !== p) fail(`${p} answered from ${r.url}, not from itself`);
  }
