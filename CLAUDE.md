...
## Next step
Phase 8 (`docs/PLAN.md`, weeks 32–36): behaviour points and rules, health and the clinic with its stock link, counselling (encrypted notes), safeguarding, special-needs plans; the LMS with courses, lessons, materials and online classes; notice scheduling, surveys and newsletters; the student PWA v1; co-curricular v1.

Phases 0–7 are implemented; each phase's status paragraph in `docs/PLAN.md` says what is done and what was left out. The plan sets no exit criterion for phases 6–9, so each is judged by its own test. `pnpm smoke` runs every phase suite (SQLite by default, `TEST_DB_URL` selects MySQL or Postgres); `PHASE4_BIG=1` runs the 1,500-student assessment exit criterion instead of the 120-student default.
