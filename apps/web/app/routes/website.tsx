  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex gap-2"><a className="btn btn-secondary btn-sm" href="/site" target="_blank" rel="noreferrer">{tr('web.view')}</a><Button size="sm" variant="secondary" onClick={() => setDrawer('page')}>{tr('web.pages')} +</Button><Button size="sm" onClick={() => setDrawer('notice')}>{tr('web.newNotice')}</Button></div>
      </div>
      ...
      <div className="mt-4 grid gap-6 xl:grid-cols-2">
        ...
        <section><h2 className="mb-2 text-base">{tr('web.pages')}</h2><DataTable locale={d.locale} searchable={false} rows={d.pages} columns={[{ key: 'title', label: tr('common.name'), render: r => <a href={Number(r.is_home) ? '/site' : `/site/${r.slug}`} target="_blank" rel="noreferrer">{String(r.title)}</a> }, { key: 'slug', label: 'Slug', className: 'num' }, { key: 'locale', label: 'Lang' }, { key: 'status', label: tr('common.status'), render: r => <Chip status={String(r.status) === 'published' ? 'active' : 'draft'}>{String(r.status)}</Chip> }]} /></section>
