...
export async function loader({ context, request }: Route.LoaderArgs) {
  if (!context.user) throw redirect(`/login?next=${encodeURIComponent('/portal')}`);
...
export default function Portal() {
  ...
  return (
    <div lang={L} className="mx-auto max-w-md pb-20">
      <header className="sticky top-0 z-10 flex items-center justify-between border-b px-4 py-3" style={{ background: 'var(--surface)', borderColor: 'var(--line)' }}><div><div className="text-xs" style={{ color: 'var(--accent)' }}>{tr('app.name')}</div><div className="display text-base">{name}</div></div><a href="/logout" className="btn btn-ghost btn-sm">{tr('nav.logout')}</a></header>
      <main className="px-4">
        ...
        <ul className="mt-2 grid gap-2">{d.children.map(c => <li key={String(c.id)}><a href={`/portal/child/${c.id}`} className="card flex items-center gap-3 p-4"><span className="feed-icon feed-system">{String(c.first_name).slice(0, 1)}</span><span className="flex-1"><span className="block font-medium">{L === 'bn' && c.name_bn ? String(c.name_bn) : `${c.first_name} ${c.last_name ?? ''}`}</span><span className="block text-xs" style={{ color: 'var(--muted)' }}>{L === 'bn' && c.class_name_bn ? String(c.class_name_bn) : String(c.class_name ?? '')} {String(c.section_name ?? '')} · {tr('stu.roll')} <span className="num">{String(c.current_roll_no ?? '—')}</span></span></span><span style={{ color: 'var(--muted)' }}>›</span></a></li>)}</ul>
      ...
      <nav className="fixed inset-x-0 bottom-0 mx-auto flex max-w-md justify-around border-t py-2 text-xs" style={{ background: 'var(--surface)', borderColor: 'var(--line)' }}><a href="/portal" style={{ color: 'var(--accent)' }}>{tr('portal.children')}</a><a href="/portal#notices">{tr('portal.notices')}</a><a href="/site">{tr('web.title')}</a></nav>
