...
export function mountPhase6(api: Router, app: App, wrap: Wrap, requirePerm: (req: Request, perm: string) => User, requireUser: (req: Request) => User) {
  ...
  api.get('/documents/issued/:id', wrap(async req => { const u = requirePerm(req, 'documents.view'); const d = await app.db.findOne('issued_documents', { id: req.params.id as string, school_id: u.school_id }); if (!d) throw new HttpError(404, 'document not found'); return d; }));
  ...
  api.get('/documents/id-cards', wrap(async req => { const u = requirePerm(req, 'documents.view'); return app.documents.idCards(u.school_id, { status: q(req, 'status'), personType: q(req, 'personType') as 'student' | 'staff' | undefined }); }));
