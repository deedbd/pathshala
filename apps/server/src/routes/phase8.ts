...
export function mountPhase8(api: Router, app: App, wrap: Wrap, requirePerm: (req: Request, perm: string) => User, requireUser: (req: Request) => User) {
  ...
  api.get('/welfare/rules', wrap(async req => { const u = requirePerm(req, 'welfare.view'); return app.welfare.behaviourRules(u.school_id); }));
  ...
  api.get('/welfare/clinic', wrap(async req => { const u = requirePerm(req, 'welfare.view'); return app.welfare.clinicVisits(u.school_id, { onDate: q(req, 'date'), studentId: q(req, 'studentId') }); }));
