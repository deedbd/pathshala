...
export function mountPhase4(api: Router, app: App, wrap: Wrap, requirePerm: (req: Request, perm: string) => User, requireUser: (req: Request) => User) {
  ...
  // ---------- question bank ----------
  api.get('/questions', wrap(async req => { const u = requirePerm(req, 'assessment.view'); return app.assessment.questions(u.school_id, { subjectId: q(req, 'subjectId'), classId: q(req, 'classId'), qType: q(req, 'type'), difficulty: q(req, 'difficulty') }); }));
  api.get('/questions/papers', wrap(async req => { const u = requirePerm(req, 'assessment.view'); return app.assessment.papers(u.school_id, { examId: q(req, 'examId') }); }));
  ...
  // ---------- online exams ----------
  api.get('/exams/online', wrap(async req => { const u = requirePerm(req, 'assessment.view'); return app.assessment.onlineExams(u.school_id, { sectionId: q(req, 'sectionId'), status: q(req, 'status') }); }));
